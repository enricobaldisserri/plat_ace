...
export function IstitutoModal({ isOpen, onClose, onSave, initialData }: IstitutoModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = initialData ? `/api/istituti/${initialData.istituto}` : "/api/istituti";
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <Card className="w-full max-w-lg overflow-hidden flex flex-col shadow-2xl">
                <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
                    <h2 className="text-xl font-bold">{initialData ? "Modifica Istituto" : "Nuovo Istituto"}</h2>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5 bg-slate-50/50 dark:bg-slate-950/50">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold uppercase text-slate-500 tracking-tight">Codice Istituto</label>
                        <input
                            type="number"
                            required
