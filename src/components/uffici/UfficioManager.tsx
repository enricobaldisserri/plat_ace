...
import { UfficioModal } from "./UfficioModal";
...
export function UfficioManager() {
    const [data, setData] = useState<Ufficio[]>([]);
    const [loading, setLoading] = useState(true);
    const [modalOpen, setModalOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<Ufficio | null>(null);

    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await fetch("/api/uffici");
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await fetch(`/api/uffici/${id}`, { method: "DELETE" });
            fetchData();
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                        <Users className="text-indigo-600" /> Gestione Uffici
                    </h1>
                    <p className="text-slate-500">Amministrazione Unità Organizzative.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {loading ? (
                                ...
                                data.map((item) => (
                                    <tr key={item.uo} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                                        <td className="px-6 py-4 font-mono font-bold text-sm text-indigo-600">{item.uo}</td>
                                        <td className="px-6 py-4 text-sm text-slate-700 dark:text-slate-300">{item.servizio}</td>
                                        <td className="px-6 py-4 text-sm text-slate-700 dark:text-slate-300 truncate max-w-xs">{item.descrizione}</td>
                                        ...
                                        <td className="px-6 py-4 text-right flex justify-end gap-2">
                                            ...
                                            <button onClick={() => handleDelete(item.uo)} className="p-2 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-lg transition-colors"><Trash2 size={16} /></button>
                                        </td>
                                    </tr>
                                ))
                            )}
