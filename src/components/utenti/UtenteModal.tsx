...
export function UtenteModal({ isOpen, onClose, onSave, initialData }: UtenteModalProps) {
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                ...
                flag_robot: initialData.flag_robot ?? false,
                uo: initialData.uo ?? "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = initialData ? `/api/utenti/${initialData.matricola}` : "/api/utenti";
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <Card className="w-full max-w-lg overflow-hidden flex flex-col shadow-2xl h-[90vh]">
                <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 shrink-0">
                    <h2 className="text-xl font-bold">{initialData ? "Modifica Utente" : "Nuovo Utente"}</h2>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5 bg-slate-50/50 dark:bg-slate-950/50">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase text-slate-500 tracking-tight">Matricola</label>
                            <input
                                type="text"
                                maxLength={10}
                                required
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase text-slate-500 tracking-tight">UO</label>
                            <input
                                type="text"
                                maxLength={5}
                                value={formData.uo}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase text-slate-500 tracking-tight">Nome</label>
                            <input
                                type="text"
                                maxLength={50}
                                value={formData.nome}
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase text-slate-500 tracking-tight">Cognome</label>
                            <input
                                type="text"
                                maxLength={50}
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold uppercase text-slate-500 tracking-tight">Email</label>
                        <input
                            type="email"
                            maxLength={256}
                    ...
                    <div className="flex gap-6 pt-2">
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={formData.flag_robot}
                                onChange={e => setFormData({ ...formData, flag_robot: e.target.checked })}
                                className="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                            />
                            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Robot</span>
                        </label>
